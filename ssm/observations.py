...
class _AutoRegressiveObservationsBase(Observations):
    ...
    def __init__(self, K, D, M=0, lags=1):
        ...
        assert lags > 0
        self.lags = lags
        self.bs = 0*npr.randn(K, D)
        self.Vs = np.eye(D,M)[None,:,:].repeat(K,axis=0)
        # npr.randn(K, D, M)
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, J0=None, h0=None):
        ...
        for (Ez, _, _), data, input, mask, tag in zip(expectations, datas, inputs, masks, tags):
            ...
            if self.fit_v:
                ...
                ys.append(data[self.lags:]-input[:-self.lags].dot(self.Vs[0].T))
        ...
        if len(unused) > 0:
            for k in unused:
                ...
                if self.fit_v: self.Vs[k] = self.Vs[i] + 0.01 * npr.randn(*self.Vs[i].shape)
